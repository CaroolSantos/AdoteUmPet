...
export class LoginPage {
  ...
  constructor(
    ...
    public accountProvider: ContasServicoProvider
  ) {
  ...
  login() {
    if (this.loginForm.valid) {
      ...
      this.accountProvider.login(this.credentials)
        .subscribe(data => {

          console.log('INFO - login > registerUser > CadastroUsuarioPage ' + JSON.stringify(data));
          //salva acces token e refresh token
          this.nativeStorage.setItem('access_token', data.access_token);
          this.nativeStorage.setItem('refresh_token', data.refresh_token);
